...
public class DNSQuestion extends DNSEntry {
    ...
    private static class DNS4Address extends DNSQuestion {
        ...
        public void addAnswers(JmDNSImpl jmDNSImpl, Set<DNSRecord> answers) {
            DNSRecord answer = jmDNSImpl.getLocalHost().getDNSAddressRecord(this.getRecordType(), DNSRecordClass.UNIQUE,
        ...
        public void addAnswers(JmDNSImpl jmDNSImpl, Set<DNSRecord> answers) {
            ...
            if (this.isServicesDiscoveryMetaQuery()) {
                for (final ServiceTypeEntry typeEntry : jmDNSImpl.getServiceTypes().values()) {
                    answers.add(new DNSRecord.Pointer("_services._dns-sd._udp.local.", DNSRecordClass.CLASS_IN,
                ...
                if ((ipValue != null) && (ipValue.length() > 0)) {
                    ...
                    if (ipValue.equalsIgnoreCase(hostIPAddress)) {
                        if (this.isV4ReverseLookup()) {
                            answers.add(jmDNSImpl.getLocalHost().getDNSReverseAddressRecord(DNSRecordType.TYPE_A,
                                    DNSRecordClass.NOT_UNIQUE, DNSConstants.DNS_TTL));
                        }
                        if (this.isV6ReverseLookup()) {
                            answers.add(jmDNSImpl.getLocalHost().getDNSReverseAddressRecord(DNSRecordType.TYPE_AAAA,
                                    DNSRecordClass.NOT_UNIQUE, DNSConstants.DNS_TTL));
                        }
                    }
                }
    ...
    private static class Service extends DNSQuestion {
        ...
        public void addAnswers(JmDNSImpl jmDNSImpl, Set<DNSRecord> answers) {
            ...
            if (jmDNSImpl.getServiceTypes().containsKey(loname)) {
                DNSQuestion question = new Pointer(this.getName(), DNSRecordType.TYPE_PTR, this.getRecordClass(),
                        this.isUnique());
    ...
    protected void addAnswersForServiceInfo(JmDNSImpl jmDNSImpl, Set<DNSRecord> answers, ServiceInfoImpl info) {
        if ((info != null) && info.isAnnounced()) {
            if (this.getName().equalsIgnoreCase(info.getQualifiedName()) || this.getName().equalsIgnoreCase(info.getType())
                ...
                answers.addAll(
                        jmDNSImpl.getLocalHost().answers(this.getRecordClass(), DNSRecordClass.UNIQUE, DNSConstants.DNS_TTL));
                answers.addAll(info.answers(this.getRecordClass(), DNSRecordClass.UNIQUE, DNSConstants.DNS_TTL,
                        jmDNSImpl.getLocalHost(), this.getSubtype()));
            }
            logger.debug("{} DNSQuestion({}).addAnswersForServiceInfo(): info: {}\n{}", jmDNSImpl.getName(), this.getName(),
                    info, answers);
        }
    }
