...
public abstract class DNSRecord extends DNSEntry {
    ...
    /**
     * True if this record would be suppressed by an answer. This is the case if this record would not have a significantly
     * longer TTL.
     */
    ...
    public static abstract class Address extends DNSRecord {
        ...
        boolean handleQuery(JmDNSImpl dns, long expirationTime) {
            if (dns.getLocalHost().conflictWithRecord(this)) {
                DNSRecord.Address localAddress = dns.getLocalHost().getDNSAddressRecord(this.getRecordType(), this.isUnique(),
    ...
    public static class Service extends DNSRecord {
        ...
        public Service(String name, DNSRecordClass recordClass, boolean unique, int ttl, int priority, int weight, int port,
                String server) {
        ...
        boolean handleQuery(JmDNSImpl dns, long expirationTime) {
            ...
            if (info != null && (info.isAnnouncing() || info.isAnnounced())
                ...
                DNSRecord.Service localService = new DNSRecord.Service(info.getQualifiedName(), DNSRecordClass.CLASS_IN,
                        DNSRecordClass.UNIQUE, DNSConstants.DNS_TTL, info.getPriority(), info.getWeight(), info.getPort(),
            ...
            if (info != null) {
                if (this._port == info.getPort() != _server.equals(dns.getLocalHost().getName())) {
                    return dns.addAnswer(in, addr, port, out,
                            new DNSRecord.Service(info.getQualifiedName(), DNSRecordClass.CLASS_IN, DNSRecordClass.UNIQUE,
