 ...
 * <p/>
 * The ServiceResolver will run only if JmDNS is in state ANNOUNCED. REMIND: Prevent having multiple service resolvers for the
 * same type in the timer queue.
 */
public class ServiceResolver extends DNSResolverTask {
    ...
    protected DNSOutgoing addAnswers(DNSOutgoing out) throws IOException {
        ...
        for (ServiceInfo info : this.getDns().getServices().values()) {
            newOut = this.addAnswer(newOut, new DNSRecord.Pointer(info.getType(), DNSRecordClass.CLASS_IN,
                    DNSRecordClass.NOT_UNIQUE, DNSConstants.DNS_TTL, info.getQualifiedName()), now);
    ...
    protected DNSOutgoing addQuestions(DNSOutgoing out) throws IOException {
        ...
        newOut = this.addQuestion(newOut,
                DNSQuestion.newQuestion(_type, DNSRecordType.TYPE_PTR, DNSRecordClass.CLASS_IN, DNSRecordClass.NOT_UNIQUE));
