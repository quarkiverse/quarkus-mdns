...
public class JmDNSTest {
    ...
    public void testListenForMyService() throws IOException {
        ...
        try {
            ...
            assertEquals("We did not get the right fully qualified name for the added service:", service.getQualifiedName(),
    ...
    public void testRegisterAndListServiceOnOtherRegistry() throws IOException, InterruptedException {
        ...
        try {
            ...
            assertEquals("Did not get the expected service fully qualified name:", service.getQualifiedName(),
                    fetchedServices[0].getQualifiedName());
    ...
    public void testTwoMulticastPortsAtOnce() throws UnknownHostException, IOException {
        ...
        try {
            ...
            DatagramPacket inSecond = new DatagramPacket(firstMessage.getBytes("UTF-8"), firstMessage.length(), someInet,
                    MPORT);
            ...
            assertEquals("Expected the second socket to recieve the same message the first socket sent", firstMessage,
                    fromFirst);
            ...
            assertEquals("Expected the first socket to recieve the same message the second socket sent", secondMessage,
                    fromSecond);
        } finally {
            if (firstSocket != null)
                firstSocket.close();
    ...
    @Test
    public void shouldNotNotifyServiceListenersForServiceResolveAfterServiceRemoved()
