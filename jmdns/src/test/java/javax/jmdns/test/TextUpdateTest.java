...
public class TextUpdateTest {
    ...
    public void setup() {
        ...
        properties.clear();
        properties.put(serviceKey, text.getBytes());
        case_sensitive_printer = ServiceInfo.create("_html._tcp.local.", "cs-printer-someuniqueid", "_Printer", 80, 0, 0, true,
                properties);
    ...
    public void testListenForTextUpdateOnOtherRegistry() throws IOException, InterruptedException {
        ...
        try {
            ...
            assertEquals("Did not get the expected service info text: ", service.getPropertyString(serviceKey),
                    result.getPropertyString(serviceKey));
    ...
    public void testRegisterCaseSensitiveField() throws IOException {
        ...
        try {
            ...
            assertEquals("Did not get the expected service info text: ", service.getPropertyString(serviceKey),
                    infos[0].getPropertyString(serviceKey));
    ...
    public void testSubtype() throws IOException {
        ...
        try {
            ...
            assertEquals("Did not get the expected service info text: ", printer.getPropertyString(serviceKey),
                    result.getPropertyString(serviceKey));
            serviceListenerMock.reset();
        } finally {
            if (registry != null)
    ...
    public void testCaseSensitiveSubtype() throws IOException {
        ...
        try {
            ...
            assertEquals("We did not get the service added event.", 1, servicesAdded.size());
            ServiceInfo info = servicesAdded.get(servicesAdded.size() - 1).getInfo();
            assertEquals("We did not get the right name for the resolved service:", case_sensitive_printer.getName(),
                    info.getName());
            assertEquals("We did not get the right type for the resolved service:", case_sensitive_printer.getType(),
                    info.getType());
            // We get the service added event when we register the service. However the service has not been resolved at this point.
            // The info associated with the event only has the minimum information i.e. name and type.
            List<ServiceEvent> servicesResolved = serviceListenerMock.servicesResolved();
            ...
            assertEquals("Did not get the expected service info subtype: ", case_sensitive_printer.getSubtype(),
                    result.getSubtype());
            assertEquals("Did not get the expected service info text: ", case_sensitive_printer.getPropertyString(serviceKey),
                    result.getPropertyString(serviceKey));
            serviceListenerMock.reset();
        } finally {
            if (registry != null)
