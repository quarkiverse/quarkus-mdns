...
IMPORTANT: `quarkus.http.host` must be set to `0.0.0.0` for the local URL `http://integration:8081` to work properly.  In dev/test mode this defaults to `localhost` which means the mDNS URL will not work.
